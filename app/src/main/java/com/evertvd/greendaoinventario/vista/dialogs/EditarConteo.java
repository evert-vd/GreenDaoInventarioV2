    ...
    public void onClick(View v) {
        if (v.getId() == R.id.btnAceptar) {
            if (validarCantidad(tilCantidad.getEditText().getText().toString())) {
                ...
                activity.editarConteo(1);
                InputMethodManager imm = (InputMethodManager) getActivity().getSystemService(INPUT_METHOD_SERVICE);
                imm.hideSoftInputFromWindow(v.getWindowToken(), 0);
                this.dismiss();
            }
        } else if (v.getId() == R.id.btnCancelar) {
            OnClickListener activity = (OnClickListener)getActivity();
            activity.editarConteo(0);
            //Conteo conteo=new Conteo();
            //conteo=null;
            //activity.editarConteo(conteo);
            InputMethodManager imm = (InputMethodManager) getActivity().getSystemService(INPUT_METHOD_SERVICE);
            imm.hideSoftInputFromWindow(v.getWindowToken(), 0);
            this.dismiss();
        }
